...
extern A_STATUS
wmi_delete_aggr_cmd (void *wmip, A_UINT8 tid, A_UINT8 uplink);
...
aggr_delba_request (void *cntxt, void *wmicntxt, A_UINT16 aid, A_UINT8 reasonCode)
{
    ...
    for (i = 0; i < NUM_OF_TIDS; i++)
    {
        ...
        if (TRUE == rxtid->aggr && 0x01 == reasonCode)
        {
            // send wmi_delba command
            // TBD::TBD:: Synchronization of continuous delba_req_event
            //
            wmi_delete_aggr_cmd (wmicntxt, muxID, 0x00);
            aggr_recv_delba_req_evt (p_aggr_conn, i);
            rxtid->bIsDelbaSent = TRUE;
        }

        if ((0x02 == reasonCode) && (TRUE == rxtid->bIsDelbaSent || i == 0))
        {
            wmi_delete_aggr_cmd (wmicntxt, muxID, 0x80);
            rxtid->bIsDelbaSent = FALSE;
        }
    }
}
...
aggr_delete_tid_state(AGGR_CONN_INFO *p_aggr_conn, A_UINT8 tid)
{
    ...
    if (tid >= NUM_OF_TIDS || p_aggr == NULL || p_aggr_conn == NULL)
...
aggr_slice_amsdu(AGGR_CONN_INFO *p_aggr_conn, RXTID *rxtid, void **osbuf)
{
    ...
    A_INT32 frame_8023_len   = 0;
    A_INT32 payload_8023_len = 0;
    A_INT32 mac_hdr_len      = 0;
    A_INT32 amsdu_len        = 0;
