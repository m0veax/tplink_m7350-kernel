              ...
              struct iw_point *erq, char *extra)
{
    ...
    A_MEMCPY(wapiKeyRsc, ext->tx_seq, IW_ENCODE_SEQ_MAX_SIZE);
    A_MEMCPY(&wapiKeyRsc[IW_ENCODE_SEQ_MAX_SIZE], ext->rx_seq, IW_ENCODE_SEQ_MAX_SIZE);
            ...
            struct iw_freq *freq, char *extra)
{
    ...
    if(ar->lteFreq && arPriv->arChannelHint && arPriv->arNetworkType == AP_NETWORK) {
        if(!CHAN_IS_11A(arPriv->arChannelHint)){
