...
extern void *ar6000_virtual_scatter_buffer_alloc(int req, int size);
extern int ar6000_virtual_scatter_buffer_free(int req);
#endif
...
static void DevCleanupVirtualScatterSupport(AR6K_DEVICE *pDev)
{
    ...
    for (i = 0; i < AR6K_SCATTER_REQS; i++) {
        status = ar6000_virtual_scatter_buffer_free(i);
        if (status) {
            AR_DEBUG_PRINTF(ATH_DEBUG_ERR, ("AR6K: %s error\n", __FUNCTION__));
        }
    }
#else
...
#ifdef USE_PREALLOC_BUFFER 
        pReq = (HIF_SCATTER_REQ *)ar6000_virtual_scatter_buffer_alloc(i,sgreqSize + bufferSize);
#else
