...
int ath6kl_wmi_ap_set_beacon_intvl_cmd(struct wmi *wmi, u8 if_idx, u32 beacon_intvl)
{
        struct sk_buff *skb;
...
{
	struct sk_buff *skb;
	struct wmi_set_max_num_cmd *cmd;

	skb = ath6kl_wmi_get_new_buf(sizeof(*cmd));
	if (!skb)
		return -ENOMEM;

	cmd = (struct wmi_set_max_num_cmd *) skb->data;
	cmd->max_num_sta = max_num_sta;

	ath6kl_dbg(ATH6KL_DBG_WMI, "ap_set_max_num_sta: max_num_sta=%d\n",
			cmd->max_num_sta);
...
int ath6kl_wmi_set_rate_ctrl_cmd(struct wmi *wmi, u8 if_idx,u32 ratemode)
{
	...
	return ath6kl_wmi_cmd_send(wmi, if_idx, skb,
			WMI_SET_RATECTRL_PARM_CMDID, NO_SYNC_WMIFLAG);
}
...
static int ath6kl_wmi_control_rx_xtnd(struct wmi *wmi, struct sk_buff *skb)
{
	...
	case WMIX_HB_CHALLENGE_RESP_EVENTID:
		...
		ath6kl_update_hb_event(wmi->parent_dev);
		break;
					...
					u8 *datap, u32 len)
{
	...
	case WMI_WLAN_INFO_LTE_EVENTID:
		...
		return ath6kl_wmi_test_rx(wmi, datap, len,
