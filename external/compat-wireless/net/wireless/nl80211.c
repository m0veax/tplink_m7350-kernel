				...
				struct cfg80211_beacon_data *bcn)
{
	...
	if (info->attrs[NL80211_ATTR_PROBE_RESP]) {
        ...
        if (info->attrs[NL80211_ATTR_STA_CAP_REQ]) {
                bcn->ap_cap_wmode = nla_get_u8(
                                      info->attrs[NL80211_ATTR_STA_CAP_REQ]);
                if (bcn->ap_cap_wmode < NL80211_AP_CAP_WMODE_11A ||
                     bcn->ap_cap_wmode > NL80211_AP_CAP_WMODE_11GONLY)
                        return -EINVAL;
        }
...
static int nl80211_trigger_scan(struct sk_buff *skb, struct genl_info *info)
{
	...
	if (info->attrs[NL80211_ATTR_HT_2040_MODE])
		request->ht_obss_scan = true;
	else
		request->ht_obss_scan = false;
...
static struct genl_ops nl80211_ops[] = {
	...
	},
	{
		...
		.doit = nl80211_set_mac_acl,.policy = nl80211_policy,
		.flags = GENL_ADMIN_PERM,
		.internal_flags = NL80211_FLAG_NEED_NETDEV |
				  NL80211_FLAG_NEED_RTNL,
	},
