						 ...
						 u8 *buf, size_t len)
{
	...
	rta_len = RTA_ALIGN(sizeof(struct rtattr));
	while (RTA_OK(attr, attrlen)) {
		if (attr->rta_type == IFLA_IFNAME && bridge_oper == 0) {
						 ...
						 u8 *buf, size_t len)
{
	...
	rta_len = RTA_ALIGN(sizeof(struct rtattr));
	while (RTA_OK(attr, attrlen)) {
		if (attr->rta_type == IFLA_IFNAME && ifi->ifi_family !=
				AF_BRIDGE) {
			wpa_driver_nl80211_event_link(
				drv,
				((char *) attr) + rta_len,
...
broken_combination:
	...
	if (tb[NL80211_ATTR_FEATURE_FLAGS]) {
		...
		if (flags & NL80211_FEATURE_MAC_ACL)
			capa->flags |= WPA_DRIVER_FLAGS_MAC_ACL;
#ifdef CONFIG_HT2040_COEX
		if (flags & NL80211_FEATURE_HT_2040_COEX)
...
#ifdef CONFIG_HT2040_COEX
	if (params->obss_scan)
		nla_put_u8(msg, NL80211_ATTR_HT_2040_MODE, 1);
#endif
				   ...
				   struct wpa_driver_scan_params *params)
{
	...
	if (ret) {
		wpa_printf(MSG_DEBUG, "nl80211: Scan trigger failed: ret=%d "
			   "(%s)", ret, strerror(-ret));
#if defined(HOSTAPD) && !defined(CONFIG_HT2040_COEX)
		/* For HT2040 Coex, scan will be done after AP bring-up so
		 * removing this part of code as this brings down the interface
		 * if the scan request fails which inturn stops the AP. As this
		 * is done for mac80211 and ath6kl supports the scan in ap so
		 * removing this part of the code for now
		 */
		if (is_ap_interface(drv->nlmode)) {
			/*
		...
		*(phy_info->num_modes) += 1;
		...
		nla_for_each_nested(nl_freq, tb_band[NL80211_BAND_ATTR_FREQS], rem_freq) {
			...
			if (tb_freq[NL80211_FREQUENCY_ATTR_RADAR])
				mode->channels[idx].flag |=
					HOSTAPD_CHAN_RADAR;
			if (tb_freq[NL80211_FREQUENCY_ATTR_NO_HT40MINUS])
				mode->channels[idx].flag |=
					HOSTAPD_CHAN_HT40MINUS;
			if (tb_freq[NL80211_FREQUENCY_ATTR_NO_HT40PLUS])
				mode->channels[idx].flag |=
...
static int nl80211_set_bss(struct i802_bss *bss, int cts, int preamble,
			   ...
			   int *basic_rates, int ht_2040_mode)
{
	struct wpa_driver_nl80211_data *drv = bss->drv;
	struct nl_msg *msg;
...
#ifdef CONFIG_HT2040_COEX
	if (ht_2040_mode > 0)
		NLA_PUT_U8(msg, NL80211_ATTR_HT_2040_MODE, ht_2040_mode);
#endif
                                     ...
                                     struct hostapd_acl_params *params)
{
	struct i802_bss *bss = priv;
	struct wpa_driver_nl80211_data *drv = bss->drv;
       struct nl_msg *msg, *acl  = NULL, *aclwild = NULL;
       u8 cmd = NL80211_CMD_SET_MAC_ACL;
       int i, ret = 0;

       if (!(drv->capa.flags & WPA_DRIVER_FLAGS_MAC_ACL))
		return -ENOTSUP;

       msg = nlmsg_alloc();
       if (!msg)
               return -ENOMEM;
       wpa_printf(MSG_DEBUG, "nl80211: Set acl, %s",
                  params->acl_policy ? "Accept acl" : "Deny acl");
	...
	for (i = 0; i < params->num_mac_acl; i++)
                NLA_PUT(acl, i + 1, ETH_ALEN, params->mac_acl[i].addr);
	...
	for (i = 0; i < params->num_mac_acl; i++)
                NLA_PUT(aclwild, i + 1, ETH_ALEN, &params->mac_acl[i].wild);

	nla_put_nested(msg, NL80211_ATTR_MAC_ADDRS_WILD, aclwild);

       NLA_PUT_U8(msg, NL80211_ATTR_ACL_POLICY, params->acl_policy ?
                  NL80211_ACL_POLICY_ACCEPT : NL80211_ACL_POLICY_DENY);
				     ...
				     struct wpa_driver_ap_params *params)
{
	...
	NLA_PUT_U32(msg, NL80211_ATTR_MAX_NUM_STA, params->max_num_sta);
	...
	switch (params->ap_capab_wmode) {
		case AP_CAP_WMODE_11A:
			NLA_PUT_U8(msg, NL80211_ATTR_STA_CAP_REQ,
					NL80211_AP_CAP_WMODE_11A);
			break;
		case AP_CAP_WMODE_NO_ADD_REQ:
		case AP_CAP_WMODE_11G:
		case AP_CAP_WMODE_11AG:
			NLA_PUT_U8(msg, NL80211_ATTR_STA_CAP_REQ,
					NL80211_AP_CAP_WMODE_11G);
			break;
		case AP_CAP_WMODE_11BONLY:
			NLA_PUT_U8(msg, NL80211_ATTR_STA_CAP_REQ,
					NL80211_AP_CAP_WMODE_11BONLY);
			break;
		case AP_CAP_WMODE_11GONLY:
			NLA_PUT_U8(msg, NL80211_ATTR_STA_CAP_REQ,
					NL80211_AP_CAP_WMODE_11GONLY);
			break;
	}

	if (drv->capa.flags & WPA_DRIVER_FLAGS_MAC_ACL)
		NLA_PUT_U8(msg, NL80211_ATTR_MAC_ACL, params->mac_acl_enable);

	if (params->auto_chan_select) {
		NLA_PUT_U8(msg, NL80211_ATTR_ACS, params->auto_chan_select);
		...
		if (params->auto_chan_select == HOSTAPD_AP_ACS_USER_DEFINED) {
			NLA_PUT_U32(msg, NL80211_ATTR_ACS_CHAN_MASK,
					params->acs_chan_mask);
		}
	}

	if (params->require_ht) {
		NLA_PUT_U8(msg, NL80211_ATTR_REQUIRE_HT, params->require_ht);
	}
...
skip_auth_type:
...
#ifdef CONFIG_IEEE80211W
	wpa_printf(MSG_DEBUG, "nl80211: MFP = %d ", params->mgmt_frame_protection);
	...
	else if (params->mgmt_frame_protection == NO_MGMT_FRAME_PROTECTION)
