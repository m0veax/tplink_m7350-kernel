...
struct wpa_driver_scan_params {
	...
	/**
	 * obss_scan - Used to trigger obss scan
	 *
	 * Used to force the OBSS scan when the STA is connected HT 40 AP
	 */
	Boolean obss_scan;
};
...
struct wpa_driver_ap_params {
	...
	u8 access_network_type;
        ...
        /**
         * ap_capab_wmode - AP wireless mode
         */
        int ap_capab_wmode;
	...
	/**
        * acl_enable - Enable/disable Mac address based ACL
        *
        * Used by drivers which advertise this capability.
        */
	int mac_acl_enable;
	...
	/**
	 * ht_capab - To configure ht capabilities
	 */
	u16 ht_capab;
	u8 auto_chan_select;
	u8 require_ht;
...
/* Driver supports mac addr acl in AP mode */
#define WPA_DRIVER_FLAGS_MAC_ACL                        0x02000000
/* Driver supports HT 20/40 coexistance */
#define WPA_DRIVER_FLAGS_HT_2040_COEX			0x04000000
	unsigned int flags;
...
struct hostapd_acl_params {
       u8 acl_policy;
       u8 num_mac_acl;
       struct mac_address_wild mac_acl[0];
};
...
struct wpa_driver_ops {
	...
	int (*set_ap)(void *priv, struct wpa_driver_ap_params *params);
       ...
       /**
        * set_acl - Set acl list in AP mode
        * @priv: Private driver interface data
        * @params: Parameters to configure acl
        * Returns: 0 on success, -1 on failure
        *
        * This is used only for the drivers which have the support of
        * doing mac address acl.
        */
    ...
    /**
     * hapd_set_hidden_ssid - Set hidden SSID (AP only)
     * @priv: Private driver interface data
     * @hid: hidden SSID ( 0 - disable, 1 - enable )
     * Returns: 0 on success, -1 on failure
     */
    int (*hapd_set_hidden_ssid)(void *priv, const u8 hid);

    /**
     * hapd_set_mac_acl - Set MAC ACL list
     * @priv: Private driver interface data
     * Returns: 0 on success, -1 on failure
     */
    int (*hapd_set_mac_acl)(void *priv);
	...
	/*
	 * Setting the hardware mode
	 * @priv: Private driver interface data
	 * @hw_mode: Hardware mode to set
	 * Returns: 0 on success, -1 on failure
	 */
	int (*hapd_set_hw_mode)(void *priv, int hw_mode);
	/*
	 * Setting intra bss mode
	 * @priv: Private driver interface data
	 * @ap_isolate: ap_isolate from configuration
	 * Returns: 0 on success, -1 on failure
	 */
	int (*hapd_set_intra_bss)(void *priv, int ap_isolate);
	/*
	 * Setting the bss params
	 * @priv: Private driver interface data
	 * @cts: Whether CTS protection is enabled
	 * @preamble: Whether short preamble is enabled
	 * @slot: Whether short slot time is enabled
	 * @ht_opmode: HT operation mode or -1 if HT not in use
	 * @ap_isolate: Whether to isolate frames between associated stations
	 * @basic_rates: -1 terminated array of basic rates in 100 kbps
	 */
	int (*set_bss)(void *priv, int cts, int preamble,
