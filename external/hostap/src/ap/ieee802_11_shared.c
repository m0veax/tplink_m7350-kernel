...
u8 *hostapd_eid_obss_scan_params(struct hostapd_data *hapd, u8 *eid)
{
	struct ieee80211_obss_scan_params_ie obss_ie;
	u16 ht_capab = hapd->iface->conf->ht_capab;
	...
	if ((ht_capab & HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET) &&
		...
		os_memset(eid, 0, sizeof(struct ieee80211_obss_scan_params_ie));
		obss_ie.eid = WLAN_EID_OVERLAPPING_BSS_SCAN_PARAMS;
		obss_ie.len = sizeof(struct ieee80211_obss_scan_params_ie) - 2;

		obss_ie.passive = OVERLAPPING_BSS_SCAN_PASS_DWELL;
		obss_ie.active = OVERLAPPING_BSS_SCAN_ACT_DWELL;

		if (hapd->iconf->obss_scan_interval)
			obss_ie.obss_scan_int = hapd->iconf->obss_scan_interval;
		else {
			obss_ie.obss_scan_int =
				OVERLAPPING_BSS_TRIGGER_SCAN_INTERVAL;
		}

		obss_ie.passive_per_ch = OVERLAPPING_BSS_PASS_TOTAL_PER_CH;
		obss_ie.active_per_ch = OVERLAPPING_BSS_ACT_TOTAL_PER_CH;
		obss_ie.ch_trns_delay = OVERLAPPING_BSS_TRANS_DELAY_FACTOR;
		obss_ie.threshold = OVERLAPPING_BSS_SCAN_ACTIVITY_THRESH;
		os_memcpy(eid, (u8 *)&obss_ie,
				sizeof(struct ieee80211_obss_scan_params_ie));
		eid += sizeof(struct ieee80211_obss_scan_params_ie);
	}
	return eid;
}
#endif
	...
	*pos++ = len;
	...
	if (hapd->iface->drv_flags & WPA_DRIVER_FLAGS_HT_2040_COEX) {
		if ((ht_capab & HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET) &&
				(hapd->iface->current_mode->mode !=
				 HOSTAPD_MODE_IEEE80211A))
			*pos++ = IEEE80211_EXTCAP_2040COEX_MGMT;
		else
			*pos++ = 0x00;
		if (len == 1)
			return pos;
