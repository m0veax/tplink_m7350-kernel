...
int hostapd_get_hw_features(struct hostapd_iface *iface)
{
	...
	for (i = 0; i < num_modes; i++) {
		...
		for (j = 0; j < feature->num_channels; j++) {
			...
			if (feature->channels[j].freq == 2484 &&
				      ...
				      struct wpa_scan_results *scan_res)
{
	...
	for (i = 0; i < scan_res->num; i++) {
		...
		if (hapd->iface->drv_flags & WPA_DRIVER_FLAGS_HT_2040_COEX) {
			struct ieee802_11_elems elems;
			ieee802_11_parse_elems((u8 *) (bss + 1), bss->ie_len,
					&elems, 0);
			if (elems.ht_capabilities) {
				...
				if (le_to_host16(ht_cap->ht_capabilities_info) &
					...
					wpa_printf(MSG_DEBUG, "40MHz Intolerant"
							" is set on channel %d"
							" in BSS "MACSTR, pri,
							MAC2STR(bss->bssid));
					hapd->ht_40 = FALSE;
				...
				wpa_printf(MSG_DEBUG, "Legacy BSS " MACSTR
						" detected on channel %d",
						MAC2STR(bss->bssid), pri);
				hapd->ht_40 = FALSE;
				return 0;
			}
		}
...
#ifdef CONFIG_HT2040_COEX
		hapd->secondary_ch = hapd->iconf->secondary_channel;
		iface->conf->secondary_channel = 0;
		if (iface->current_mode->mode == HOSTAPD_MODE_IEEE80211A)
			iface->conf->ht_capab &=
				~HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET;
		eloop_cancel_timeout(ap_ht2040_timeout,	hapd, NULL);
		eloop_register_timeout(delay_time, 0, ap_ht2040_timeout, hapd,
				NULL);
#else
		iface->conf->secondary_channel = 0;
...
#ifdef CONFIG_HT2040_COEX
	if((iface->conf->auto_chan_select >= HOSTAPD_AP_ACS_NORMAL &&
		iface->conf->auto_chan_select < NUM_HOSTAPD_AP_ACS_TYPES)){
			if (!oper40){
				wpa_printf(MSG_INFO, "Fallback to 20 MHz");
				hostapd_drv_set_bss(hapd, -1, -1, -1, -1, -1,
						NULL, HT_OPMODE_SWITCH_TO_20);
			}
	} else {
#endif
...
static void ieee80211n_scan_channels(struct hostapd_iface *iface,
...
#define  HT2040_COEX_SCAN_RETRY 15
	...
	if (iface->conf->ht2040_coex_disable)
		return 0;
	...
	ieee80211n_scan_channels(iface, &params);
	...
	do {
		...
		if (ret < 0) {
			if (ret == -EBUSY) {
				sleep(1);
				wpa_printf(MSG_ERROR, "Failed to request a scan"
					       " of neighboring BSSes scanning"
					       " again ret=%d (%s)", ret,
					       strerror(-ret));
				continue;
			} else {
				wpa_printf(MSG_ERROR, "Failed to request a scan"
					       " of neighboring BSSes ret=%d "
					       "(%s)", ret, strerror(-ret));
		...
		break;
	} while (++i < HT2040_COEX_SCAN_RETRY);

	if (i == HT2040_COEX_SCAN_RETRY) {
		wpa_printf(MSG_DEBUG, "Failed to request a scan in device, "
				" Bringing up in HT 20 mode ");
		iface->conf->secondary_channel = 0;
		iface->conf->ht_capab &= ~HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET;
		if(iface->conf->auto_chan_select >= HOSTAPD_AP_ACS_NORMAL
			&& hapd->iface->conf->auto_chan_select <
			NUM_HOSTAPD_AP_ACS_TYPES) {
			/* In ACS case set_up interface is already done so only
			 * set_bss to driver to fall to HT20 mode is enough
			 * */
			hostapd_drv_set_bss(hapd, -1, -1, -1, -1, -1, NULL,
					HT_OPMODE_SWITCH_TO_20);
		}
		/* Here if we return 0 caller will take care of bringing in
		 * HT20 mode.
		 * */
		os_free(params.freqs);
		return 0;
...
int hostapd_check_ht_capab(struct hostapd_iface *iface)
{
	...
	if((iface->conf->auto_chan_select >= HOSTAPD_AP_ACS_NORMAL &&
		iface->conf->auto_chan_select < NUM_HOSTAPD_AP_ACS_TYPES)
		&& (!iface->freq) && iface->conf->secondary_channel != 0) {
		/* by pass obss scan in case of ACS*/
		hostapd_setup_interface_complete(iface, 0);
	} else {
...
int hostapd_select_hw_mode(struct hostapd_iface *iface)
{
	...
	if (iface->conf->secondary_channel > 0) {
		if (pri_chan_flag & HOSTAPD_CHAN_HT40PLUS) {
			...
			hostapd_logger(iface->bss[0], NULL,
					...
					"of current mode (%d) %s,\nSwitching to"
					" HT 20MHz operatin mode",
					sec_chan, iface->current_mode->mode,
					hostapd_hw_mode_txt(
						iface->current_mode->mode));
		}
	} else if (iface->conf->secondary_channel < 0) {
		if (pri_chan_flag & HOSTAPD_CHAN_HT40MINUS) {
			int sec_chan = iface->conf->channel +
				iface->conf->secondary_channel * 4;
			iface->conf->secondary_channel = 0;
			iface->conf->ht_capab &=
