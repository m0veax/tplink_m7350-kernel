...
extern int ar6000_set_max_num_sta(void *priv, const u8 num_sta);
...
void hostapd_clear_wep(struct hostapd_data *hapd)
{
	if (hapd->drv_priv) {
		hostapd_set_privacy(hapd, 0);
...
static int hostapd_setup_bss(struct hostapd_data *hapd, int first)
{
    ...
    if (hostapd_set_hidden_ssid(hapd, conf->ignore_broadcast_ssid)) {
        wpa_printf(MSG_ERROR, "Could not set hidden SSID mode");
        return -1;
    }
    if (hostapd_set_mac_acl(hapd)) {
        wpa_printf(MSG_ERROR, "Could not set mac ACL");
        return -1;
    }
                               ...
                               int n_entries, u8 accept_acl)
{
       ...
       for (i = 0; i < n_entries; i++) {
               memcpy(acl_params->mac_acl[i].addr, mac_acl[i].addr, ETH_ALEN);
               acl_params->mac_acl[i].wild = mac_acl[i].wildcard;
	}
...
int hostapd_setup_interface_complete(struct hostapd_iface *iface, int err)
{
	...
	if(hapd->conf->isolate) {
		if(hostapd_set_intra_bss(hapd, !hapd->conf->isolate) < 0)
		{
			wpa_printf(MSG_ERROR, "%s: Failed to set ap_isolate", __func__);
			return -1;
		}
	}

	if(NUM_HOSTAPD_MODES != hapd->iconf->hw_mode){
		if(hostapd_set_hw_mode(hapd, hapd->iconf->hw_mode)  < 0)
		{
			wpa_printf(MSG_ERROR,"hostapd_set_hw_mode() is failed");
		}
	}
...
#ifdef CONFIG_DRIVER_AR6000
	if ((iface->conf->driver != NULL) &&
		(os_strcmp(iface->conf->driver->name, wpa_driver_ar6000_ops.name) == 0))
	{
		if (ar6000_set_max_num_sta(hapd->drv_priv, hapd->conf->max_num_sta) < 0) {
		wpa_printf(MSG_ERROR, "%s: Failed to set max sta", __func__);
		return -1;
		}
	}
#endif
