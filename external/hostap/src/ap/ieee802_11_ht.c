...
static int is_40_allowed(struct hostapd_iface *iface, int channel)
{
	...
	if (iface->conf->secondary_channel > 0)
		...
		sec_freq = pri_freq - 20; /* calculating the secondary channel
		...
		const struct ieee80211_mgmt *mgmt, size_t len)
{
	...
	hostapd_logger(hapd, mgmt->sa, HOSTAPD_MODULE_IEEE80211,
			HOSTAPD_LEVEL_DEBUG, "hostapd_public_action - "
			" IE%x", mgmt->u.action.u.public_action.action);
	...
	if (hapd->iface->conf->ht_capab & HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET) {
		...
		if (bc_ie->coex_param & WLAN_20_40_BSS_COEX_20MHZ_WIDTH_REQ) {
			hostapd_logger(hapd, mgmt->sa,
					...
					HOSTAPD_LEVEL_DEBUG,
					"20 Mhz BSS width request bit "
					" is set in BSS coexistence "
					"information field");
			isht_allowed = 0;
		}
		...
		if (bc_ie->coex_param & WLAN_20_40_BSS_COEX_40MHZ_INTOL) {
			hostapd_logger(hapd, mgmt->sa,
					HOSTAPD_MODULE_IEEE80211,
					HOSTAPD_LEVEL_DEBUG,
					"40 Mhz intolerant bit "
					" is set in BSS coexistence "
					"information field");
			isht_allowed = 0;
		}

		if (ic_report && (ic_report->element_id ==
					WLAN_EID_20_40_BSS_INTOLERANT)) {
		/* Going through the channel report to find any bss are
		 * there in affected channel range */
			for (i = 0; i < (ic_report->length - 1); i++) {
				if (!is_40_allowed(hapd->iface,
		...
		if (!isht_allowed) {
			...
			if (!hapd->intolerant_client) {
				eloop_cancel_timeout(ap_ht2040_timeout, hapd,
						NULL);
				eloop_register_timeout(delay_time, 0,
						ap_ht2040_timeout, hapd, NULL);
			}
			hapd->ht_40 = FALSE;
		}
	}
