...
#define BEACON_TAIL_BUF_SIZE 512
	...
	*pos++ = WLAN_EID_SSID;
	...
	params.mac_acl_enable = hapd->conf->mac_acl_enable;
	params.ap_capab_wmode = hapd->conf->ap_capab_wmode;
	params.ap_max_inactivity = hapd->conf->ap_max_inactivity;
#ifdef CONFIG_HS20
	params.disable_dgaf = hapd->conf->disable_dgaf;
#endif /* CONFIG_HS20 */

	if(hapd->iconf->ht_enable) {
		params.ht_capab = hapd->iconf->ht_capab;
	}

	if ((params.ht_opmode >= 0) && (hapd->iconf->require_ht)) {
		params.require_ht = hapd->iconf->require_ht;
	}

	params.auto_chan_select = hapd->iconf->auto_chan_select;
	params.max_num_sta = hapd->conf->max_num_sta;
	params.acs_chan_mask = hapd->iconf->acs_chan_mask;
