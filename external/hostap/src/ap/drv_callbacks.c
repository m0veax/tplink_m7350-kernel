...
skip_wpa_check:
	...
	if (hapd->iface->drv_flags & WPA_DRIVER_FLAGS_HT_2040_COEX) {
		if ((elems.ht_capabilities) && (hapd->iface->conf->ht_capab &
			...
			if (le_to_host16(ht_cap->ht_capabilities_info) &
				...
				wpa_printf(MSG_INFO, "Forty MHz Intolerant"
						" is set by STA " MACSTR
						" in ASSOC REQ", MAC2STR(addr));
				hapd->iconf->secondary_channel =
					hapd->secondary_ch;
				hapd->ht_40 = FALSE;
				eloop_cancel_timeout(ap_ht2040_timeout, hapd,
						NULL);
				hapd->intolerant_client |= (1 << sta->aid);
			}
		}
	}
#endif
...
#ifdef CONFIG_HT2040_COEX
	hapd->intolerant_client &= ~(1 << sta->aid);
	if (!hapd->intolerant_client && (hapd->iconf->ht_capab &
			HT_CAP_INFO_SUPP_CHANNEL_WIDTH_SET)) {
			     ...
			     int offset)
{
	...
	if((ht) && (hapd->iface->conf->auto_chan_select >= 
			HOSTAPD_AP_ACS_NORMAL && 
			hapd->iface->conf->auto_chan_select < 
			NUM_HOSTAPD_AP_ACS_TYPES)){
		hostapd_check_ht_capab(hapd->iface);
		wpa_printf(MSG_DEBUG, "Channel : %d", channel);
		wpa_printf(MSG_DEBUG, "offset : %d", offset);
		wpa_printf(MSG_DEBUG, "HT CAPABILITY  : %s",(offset != 0)?((offset == 1)?"HT40PLUS":"HT40MINUS"):((ht)?"HT20":"NOHT"));
	}
...
static int hostapd_iface_enable(struct hostapd_iface *hapd_iface)
{
...
static int hostapd_iface_disable(struct hostapd_iface *hapd_iface)
{
	size_t j;
	const struct wpa_driver_ops *driver;
	...
	if (hapd_iface->conf->auto_chan_select >= HOSTAPD_AP_ACS_NORMAL &&
		hapd_iface->conf->auto_chan_select < NUM_HOSTAPD_AP_ACS_TYPES) {
		struct hostapd_data *hapd = hapd_iface->bss[0];
		hapd->iconf->channel = 0;
		hapd_iface->freq = 0;
	}
...
static void hostapd_event_interface_status(struct hostapd_data *hapd,
					   union wpa_event_data *data)
{
	struct hostapd_iface *hapd_iface = hapd->iface;
	struct hapd_interfaces *interfaces;
	int interface_found = 0, i;
	...
	if (interface_found == 0) {
		wpa_printf(MSG_ERROR, "No existing interface"
			   "found %s", __func__);
		return;
	}
	...
	switch (data->interface_status.ievent) {
		case EVENT_INTERFACE_ADDED:
			if (hostapd_iface_enable(hapd_iface))
				wpa_printf(MSG_ERROR, "hostapd interface add"
					   "failed");
			break;
		case EVENT_INTERFACE_REMOVED:
			if (hostapd_iface_disable(hapd_iface))
				wpa_printf(MSG_ERROR, "hostapd interface remove"
					   "failed");
			break;
	}
			  ...
			  union wpa_event_data *data)
{
	...
	case EVENT_INTERFACE_STATUS:
		hostapd_event_interface_status(hapd, data);
		break;
	default:
