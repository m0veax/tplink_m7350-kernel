...
static int wps_build_probe_config_methods(struct wps_registrar *reg,
					  struct wpabuf *msg)
{
    wpa_printf(MSG_DEBUG, "WPS:  * Config Methods (%x)", reg->wps->config_methods);
	wpabuf_put_be16(msg, ATTR_CONFIG_METHODS);
	wpabuf_put_be16(msg, 2);
	wpabuf_put_be16(msg, reg->wps->config_methods);
	return 0;
}
...
int wps_build_cred(struct wps_data *wps, struct wpabuf *msg)
{
	...
	if (wps->wps->wps_state == WPS_STATE_NOT_CONFIGURED && wps->wps->ap &&
        ...
        sprintf(newssid, "Network-%02x%02x%02x%02x%02x%02x",
                wps->wps->dev.mac_addr[0], wps->wps->dev.mac_addr[1],
                wps->wps->dev.mac_addr[2], wps->wps->dev.mac_addr[3],
                wps->wps->dev.mac_addr[4], wps->wps->dev.mac_addr[5]);
        wps->wps->ssid_len = strlen(newssid);
        os_memcpy(wps->wps->ssid, newssid, wps->wps->ssid_len);
        os_memcpy(wps->cred.ssid, wps->wps->ssid, wps->wps->ssid_len);
        wps->cred.ssid_len = wps->wps->ssid_len;
		...
		os_free(wps->new_psk);
        wps->new_psk_len = wps->wps->network_key_len;
...
static int wps_process_auth_type_flags(struct wps_data *wps, const u8 *auth)
{
	...
	wpa_printf(MSG_DEBUG, "WPS: Enrollee Authentication Type flags 0x%x",
		   auth_types);
    /* WAR for JS 2.0.0.275, refer EV56323 */
