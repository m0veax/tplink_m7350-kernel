...
static int hostapd_ctrl_iface_set_vendorie(struct hostapd_data *hapd, char *cmd)
{
    int ie_len = 0;
    u8 *ie_bin = NULL;
    char *txt = NULL;
    ...
    if (os_strncmp(cmd, "set ", 4) == 0) {

        txt = cmd+4;
        ie_len = os_strlen(txt);
        ...
        if(hapd->vendor_ie){
            wpabuf_free(hapd->vendor_ie);
        }
        hapd->vendor_ie = NULL;

        hapd->vendor_ie = wpabuf_alloc(ie_len);
        if(hapd->vendor_ie == NULL){
            retval = -1;
            goto exit;
        }

        if(hexstr2bin(txt, ie_bin, ie_len) < 0){
            retval = -1;
            goto exit;
        }

        wpabuf_put_data(hapd->vendor_ie, ie_bin, ie_len);
    }else if (os_strncmp(cmd, "clr", 3) == 0){
        if(hapd->vendor_ie){
            wpabuf_free(hapd->vendor_ie);
        }
        hapd->vendor_ie = NULL;
				       ...
				       void *sock_ctx)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		reply_len = hostapd_ctrl_iface_get(hapd, buf + 4, reply,
						   reply_size);
    } else if (os_strncmp(buf, "VENDORIE ", 9) == 0){
        if (hostapd_ctrl_iface_set_vendorie(hapd, buf+9))
            reply_len = -1;
