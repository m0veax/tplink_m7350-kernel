...
#define NL80211_MAX_STA_COUNT 15;
#define AR6000_MAX_STA_COUNT 10;
...
/* This variable is used to just validate the global_max_num_sta value set in the config file.
   The actual value is set and used in bss->max_num_sta */
static int global_max_num_sta_value = -1;
				       ...
				       struct mac_acl_entry **acl, int *num)
{
	...
	while (fgets(buf, sizeof(buf), f)) {
		...
		if (!hwaddr_aton_wild(buf, addr, &wild)) {
...
static u32 hostapd_config_acs_chan_mask(struct hostapd_config *conf,
		const char *acs_chan_mask)
{
	const char delimeter[2] = ",";
	const char *token = os_strtok(acs_chan_mask, delimeter);

	while (token != NULL) {
		if (!os_strcmp(token, "0"))
			conf->acs_chan_mask = 0;
		else if (!os_strcmp(token, "1"))
			conf->acs_chan_mask |= 1 << 0;
		else if (!os_strcmp(token, "2"))
			conf->acs_chan_mask |= 1 << 1;
		else if (!os_strcmp(token, "3"))
			conf->acs_chan_mask |= 1 << 2;
		else if (!os_strcmp(token, "4"))
			conf->acs_chan_mask |= 1 << 3;
		else if (!os_strcmp(token, "5"))
			conf->acs_chan_mask |= 1 << 4;
		else if (!os_strcmp(token, "6"))
			conf->acs_chan_mask |= 1 << 5;
		else if (!os_strcmp(token, "7"))
			conf->acs_chan_mask |= 1 << 6;
		else if (!os_strcmp(token, "8"))
			conf->acs_chan_mask |= 1 << 7;
		else if (!os_strcmp(token, "9"))
			conf->acs_chan_mask |= 1 << 8;
		else if (!os_strcmp(token, "10"))
			conf->acs_chan_mask |= 1 << 9;
		else if (!os_strcmp(token, "11"))
			conf->acs_chan_mask |= 1 << 10;
		else if (!os_strcmp(token, "12"))
			conf->acs_chan_mask |= 1 << 11;
		else if (!os_strcmp(token, "13"))
			conf->acs_chan_mask |= 1 << 12;
		else {
			conf->acs_chan_mask = 0xffff;
			break;
		}

		token = os_strtok(NULL, delimeter);
	}

	return conf->acs_chan_mask;
}
...
{
	Boolean is_ar6000 = FALSE;
#ifdef CONFIG_DRIVER_AR6000
	if ((conf->driver != NULL) &&
		(os_strcmp(conf->driver->name, wpa_driver_ar6000_ops.name) == 0))
			is_ar6000 = TRUE;
#endif /* CONFIG_DRIVER_AR6000 */
	...
	if(!is_ar6000) {
		if (!conf->ap_capab_wmode)
			conf->ap_capab_wmode = AP_CAP_WMODE_11G;

		if (conf->ap_capab_wmode) {
			switch (conf->ap_capab_wmode) {
			case AP_CAP_WMODE_11A:
				conf->hw_mode = HOSTAPD_MODE_IEEE80211A;
				break;
			case AP_CAP_WMODE_11BONLY:
				conf->hw_mode = HOSTAPD_MODE_IEEE80211B;
				break;
			case AP_CAP_WMODE_NO_ADD_REQ:
			case AP_CAP_WMODE_11G:
			case AP_CAP_WMODE_11AG:
			case AP_CAP_WMODE_11GONLY:
				conf->hw_mode = HOSTAPD_MODE_IEEE80211G;
				break;
			}
		}
		bss->ap_capab_wmode = conf->ap_capab_wmode;
	}
	...
	if (!conf->ieee80211n || bss->disable_11n) {
		conf->ieee80211n = 0;
		conf->ht_capab = 0;
	}
    ...
    if (bss->wps_state && bss->macaddr_acl > DISABLE_MAC_ACL) {
        wpa_printf(MSG_INFO, "WPS: ACL configuration forced WPS to be "
                "disabled");
        bss->wps_state = 0;
    }
#endif /* CONFIG_WPS2 */
	...
	int max_num_sta_allowed = NL80211_MAX_STA_COUNT;
	if(is_ar6000)
		max_num_sta_allowed = AR6000_MAX_STA_COUNT;

	/* If max_num_sta value is not specified in config file */
	if(bss->max_num_sta == -1)
		bss->max_num_sta = max_num_sta_allowed;
	else {
		/*If global_max_num_sta value is set in the config file */
		if(global_max_num_sta_value != -1) {
			if(is_ar6000) {
				if ((global_max_num_sta_value < -1) ||
					(global_max_num_sta_value > max_num_sta_allowed)) {
					wpa_printf(MSG_ERROR, "Invalid "
					"global_max_num_sta=%d; allowed range "
					"0..%d", global_max_num_sta_value, max_num_sta_allowed);

					global_max_num_sta_value = -1;
				...
				/* global_max_num_sta is not supported in MDM9x25 */
				wpa_printf(MSG_ERROR, "Invalid argument. "
				"global_max_num_sta is not supported.");
			...
			int max_num_sta = bss->max_num_sta ;
			if ((max_num_sta < -1) ||
				(max_num_sta > max_num_sta_allowed)) {
				wpa_printf(MSG_ERROR, "Invalid "
				"max_num_sta=%d; allowed range "
				"0..%d", max_num_sta, max_num_sta_allowed);
	...
	if (conf->auto_chan_select)
		conf->channel = 0;
	if (conf->acs_chan_mask &&
			conf->auto_chan_select != HOSTAPD_AP_ACS_USER_DEFINED) {
		wpa_printf(MSG_ERROR, "Invalid acs_chan_mask, it can only be"
				" configured when auto_chan_select = 6(means"
				" enabling oevrlapping ACS");
		return -1;
	}
	if ((conf->ap_capab_wmode == AP_CAP_WMODE_11A) &&
		(conf->auto_chan_select == HOSTAPD_AP_ACS_USER_DEFINED)) {
		wpa_printf(MSG_ERROR, "Invalid auto_chan_select value, "
				" Overlapping ACS can only be selected for g "
				" mode wmode=2 \n");
		return -1;
	}

	if (!conf->acs_chan_mask && conf->auto_chan_select ==
			HOSTAPD_AP_ACS_USER_DEFINED) {
		wpa_printf(MSG_INFO, "user defined acs is selected and "
				"acs_chan_mask value is 0, so enabling ACS "
				"for all the channels");
		conf->acs_chan_mask = 0x1fff;
	}
			       ...
			       char *buf, char *pos, int line)
{
...

	{
		if (os_strcmp(buf, "interface") == 0) {
			...
			if (bss->macaddr_acl != ACCEPT_UNLESS_DENIED &&
			    bss->macaddr_acl != DENY_UNLESS_ACCEPTED &&
                    bss->macaddr_acl != DISABLE_MAC_ACL &&
			    bss->macaddr_acl != USE_EXTERNAL_RADIUS_AUTH) {
				wpa_printf(MSG_ERROR, "Line %d: unknown "
					   "macaddr_acl %d",
					   line, bss->macaddr_acl);
			}
			if (bss->macaddr_acl != USE_EXTERNAL_RADIUS_AUTH)
				bss->mac_acl_enable = 1;
		...
		global_max_num_sta_value = atoi(pos);
            bss->max_num_sta = atoi(pos);
            bss->max_num_sta |= 0x80;
			...
			if (os_strcmp(pos, "a") == 0)
                ...
                } else if (os_strcmp(buf, "wmode") == 0) {
                        conf->ap_capab_wmode = atoi(pos);
                        if ((conf->ap_capab_wmode < 1) || (conf->ap_capab_wmode > 6))
                        {
                                wpa_printf(MSG_DEBUG, "unknown wmode = %d , resetting to 0\n",conf->ap_capab_wmode);
                                conf->ap_capab_wmode = 0;
                        }
...
#ifdef CONFIG_HT2040_COEX
		} else if (os_strcmp(buf, "obss_scan_interval") == 0) {
			int val = atoi(pos);
			conf->obss_scan_interval = val;

			if (conf->obss_scan_interval >
					HT_2040_MAX_SCAN_INTERVAL) {
				conf->obss_scan_interval =
					HT_2040_MAX_SCAN_INTERVAL;

				wpa_printf(MSG_INFO, "obss_scan_interval is "
							"greater than MAX value"
						", assigning MAX scan interval"
						"interval = %d sec",
						conf->obss_scan_interval);
			}
		} else if (os_strcmp(buf, "ht2040_coex_disable") == 0) {
			int val = atoi(pos);
			conf->ht2040_coex_disable = (u8)val;

			if (conf->ht2040_coex_disable > 1) {
				wpa_printf(MSG_INFO, "ht2040_coex_disable: "
						"invalid value :%d ",
						conf->ht2040_coex_disable);
				errors++;
			}
#endif
		} else if (os_strcmp(buf, "auto_chan_select") == 0) {
			int val = atoi(pos);
			if ( val >= HOSTAPD_AP_ACS_NORMAL &&
				    val < NUM_HOSTAPD_AP_ACS_TYPES) {
				conf->auto_chan_select = val;
				...
				wpa_printf(MSG_ERROR, "Line %d: invalid "
                                           "auto_chan_select %d (expected "
                                           "1..6)", line, val);
                                errors++;
			}
		} else if (os_strcmp(buf, "acs_chan_list") == 0) {
			u32 val = hostapd_config_acs_chan_mask(conf, pos);
			if (val > 0x1fff) { /* 8192, because only till
						      channel 13 (means only
						      till bit 13) we can set
						      channel mask*/
				wpa_printf(MSG_INFO, "Invalid channel mask "
						"value, channel mask can be "
						"given only till channel 13\n"
						"Line :: %d\n",line);
				errors++;
			}
...
struct hostapd_config * hostapd_config_read(const char *fname)
{
	...
	/* set the hw_mode default to 11g mode, if this param is not
	   present or commented in the hostapd.conf file. */
	conf->hw_mode = NUM_HOSTAPD_MODES;
    ...
    if(bss->ssid.security_policy == SECURITY_WPA_PSK) {
        char hex[65];
        if(!bss->ssid.ssid_set) {
            os_memcpy(bss->ssid.ssid, hex, bss->ssid.ssid_len);
            bss->ssid.ssid[bss->ssid.ssid_len] = '\0';
        }

        if((bss->ssid.wpa_psk == NULL) && (bss->ssid.wpa_passphrase == NULL)
                && bss->wps_state) {
            ...
            else if (os_get_random(bss->ssid.wpa_psk->psk, 32) < 0) {

            }
            wpa_snprintf_hex(hex, sizeof(hex), bss->ssid.wpa_psk->psk, 32);
            wpa_printf(MSG_ERROR, "WPS:Self-generated PSK:\n%s", hex);
            bss->ssid.wpa_psk->group = 1;

            wpa_printf(MSG_DEBUG,"File name ********* %s",fname);
            f = fopen(fname, "a");
            if (f == NULL) {
                wpa_printf(MSG_ERROR, "Could not open configuration file '%s' "
                        "for writing.", fname);
